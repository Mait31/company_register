...
当前目标是用一套响应式网站，把公司注册前置资料和委托书材料收集标准化，并基于 Word 模板生成委托书 DOCX 内部草稿。做到委托书模板生成后，本项目先暂停。

```text
创建定向邀请 -> 客户入口填写 -> 内部核对资料 -> 资料归档 -> 委托书材料收集 -> 材料审核 -> 基于 Word 模板生成委托书 DOCX 内部草稿 -> 项目暂停
```

当前核心对象是：公司注册邀请资料台账和委托书材料。

当前最终产物是：可供内部核对和线下公证准备使用的委托书 DOCX 内部草稿。

入口层原则：

- 微信 H5 只是客户入口前端，不是真正的工单系统。
- 当前不规划小程序、普通网页、渠道端等新入口。
- 所有入口都必须调用统一后端 API，沉淀到同一套邀请、资料、工单和公司档案模型。
- 材料审核和委托书生成逻辑只能在后端业务层实现，不能写死在某个入口前端里。
...
- 客户、公司、法人、股东资料收集。
- 委托书材料上传、审核、驳回、补材料。
- 前置资料和委托书材料审核通过后直接基于 Word 模板生成委托书 DOCX 内部草稿。
- 网站后台统一管理内部业务。
...
不属于当前目标的需求，不记录为当前待办，不直接混入项目。

## 3. 用户角色

- 客户：通过公开 token 链接填写资料、上传委托书材料、查看补材料要求。
...
- 数据库迁移：Alembic。
- 文件存储：服务器本地挂载目录。
- DOCX：Word 模板自动填充；当前不推进 PDF 转换。
- 登录：网站后台统一管理；当前不推进企业微信集成。
...
第一阶段前端主链路不要求先创建正式 `registration_order`。微信卡片、H5、未来小程序等入口只创建或补充邀请资料；内部人员完成前置资料归档并审核三项委托书材料后，直接在邀请资料台账中基于 Word 模板生成委托书 DOCX 内部草稿。

当前阶段只关注：

```text
邀请资料状态：资料待核对 / 资料已归档
委托书材料状态：待收集 / 待上传 / 待审核 / 已通过 / 已驳回
委托书生成状态：未生成 / 已生成 / 生成失败或缺字段
```

状态规则：

- 生成委托书 DOCX 内部草稿前，邀请资料必须已归档，三项委托书材料必须审核通过。
- 当前阶段不推进注册办理。
- 当前阶段不推进注册结果上传或公司归档。
...
- `persons`：自然人资料。
- `shareholders`：股东。
- `quotations`：报价单底座，当前不使用。
- `quotation_items`：报价明细底座，当前不使用。
...
- `/api/admin/orders/*` 是历史正式工单底座，当前前端主流程不依赖转正式工单。
- 报价相关接口不进入第一阶段主流程。
...
```text
企业微信 API 当前不作为主链路；如已有接口保留为历史底座，不作为当前任务。
```
...
```text
内部创建注册邀请 -> 生成邀请链接/二维码/微信卡片 -> 指定客户打开 -> 填写基础资料 -> 内部核对并标记资料归档 -> 发起委托书材料收集 -> 客户上传材料 -> 内部审核材料 -> 基于 Word 模板生成委托书 DOCX 内部草稿
```
...
- 内部人员确认基础资料并审核委托书材料通过后，邀请资料可直接生成委托书 DOCX 内部草稿。
- 当前不规划小程序入口。
...
当前阶段按以下顺序实施，到委托书 DOCX 内部草稿生成为止：
...
8. 前置资料和委托书材料审核通过后基于 Word 模板生成委托书 DOCX 内部草稿。
9. 完成委托书生成链路验收后暂停项目。
...
7. 客户上传委托书材料。
8. 内部人员审核材料并标记通过或驳回。
9. 三项材料审核通过后，内部人员直接生成委托书 DOCX 内部草稿。
10. 系统保存生成文件记录，并提示缺失字段。
11. 内部人员可下载或查看内部草稿，用于线下公证准备。
12. 项目暂停，不继续推进付费、注册办理、结果上传或公司归档。
```
