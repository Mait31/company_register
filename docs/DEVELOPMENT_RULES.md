...
目标是用一套响应式网站，把公司注册从人工问材料、手工报价、手工整理文件，改造成标准化工单流程：

```text
创建定向邀请 -> 客户入口填写 -> 内部确认 -> 创建正式工单 -> 报价确认 -> 材料上传 -> 材料审核 -> 文件生成 -> 注册办理 -> 结果上传 -> 公司归档
```
...
- 注册结果文件上传。
- 公司档案归档。
- 企业微信自建应用登录、通讯录同步、消息通知。
- 微信公众号 H5 定向邀请入口，用于精准客户资料采集。
...
正式公司注册工单只能由内部确认后创建。微信 H5、未来小程序等入口只创建或补充邀请资料，不直接创建正式工单。
...
GET    /api/public/orders/{token}/quotation
POST   /api/public/orders/{token}/confirm-quotation
...
GET    /api/admin/orders/{id}/quotations
...
POST   /api/admin/invitations/{id}/convert-to-order
```
...
```text
内部创建注册邀请 -> 生成 H5 邀请链接/二维码 -> 指定客户微信打开 -> 填写资料/上传材料 -> 内部确认 -> 转为正式注册工单
```
...
- 邀请参与人只能补充资料，不能直接改变正式工单状态。
- 内部人员确认后，邀请资料才转为正式 `registration_orders`。
...
1. 内部人员通过企业微信登录。
2. 内部人员创建公司注册定向邀请。
3. 系统生成微信公众号 H5 邀请链接或二维码。
4. 指定客户用微信打开链接并完成网页授权。
5. 客户填写公司、法人、股东信息。
6. 客户可转给法人或股东补充资料，系统记录参与人。
7. 客户上传护照、签证、签名等材料。
8. 内部人员确认邀请资料并转为正式公司注册工单。
9. 内部人员生成报价单。
10. 客户确认报价。
11. 内部人员审核材料并标记材料已齐。
12. 系统根据模板生成注册登记表、授权书、租房合同等 PDF。
13. 注册办理员推进工单到注册办理中。
14. 注册完成后上传注册结果文件。
15. 系统生成结构化公司档案。
16. 企业微信发送关键节点通知。
