...
```text
创建定向邀请 -> 客户入口填写 -> 内部核对资料 -> 资料归档 -> 委托书材料收集 -> 材料审核 -> 创建正式工单 -> 付费确认/办理启动 -> 文件生成 -> 注册办理 -> 结果上传 -> 公司最终归档
```
...
正式公司注册工单只能在前置资料归档、三项委托书材料审核通过后由内部人员创建。微信卡片、H5、未来小程序等入口只创建或补充邀请资料，不直接创建正式工单。
...
- 转正式工单前，邀请资料必须已归档，三项委托书材料必须审核通过。
- 材料未齐，不能进入注册办理。
- 注册结果文件未上传，不能归档。
...
- `/api/admin/orders/*` 是正式工单底座，前端主流程将在前置资料转正式工单后逐步接入。
- 报价相关接口不进入第一阶段主流程。
...
- 内部人员确认基础资料并审核委托书材料通过后，邀请资料才转为正式 `registration_orders`。
...
7. 客户上传委托书材料。
8. 内部人员审核材料并标记通过或驳回。
9. 三项材料审核通过后，内部人员转为正式公司注册工单。
10. 系统回写邀请的 `order_id`，写入 `workflow_logs` 和 `audit_logs`。
11. 内部人员完成付费确认或办理启动。
