...
- 每次修改 `AGENTS.md`、`memory.md` 或 `README.md` 后，必须更新 Notion 中对应文件的全文副本，并在本项目同步页追加同步记录。

## MVP 硬边界

当前只做到公司注册委托书模板生成：响应式网站、内部后台、客户邀请填写页、资料归档、委托书材料收集审核、基于 Word 模板的委托书 DOCX 内部草稿生成。委托书生成链路稳定后，项目暂停。
...
- 用户已明确：`company` 项目做到委托书模板生成就 OK，其他计划全部不要，先聚焦。
